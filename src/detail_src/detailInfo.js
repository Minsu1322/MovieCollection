...
export const detailInfo = async () => {
  ...
  fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${castApiKey}`, castOptions)
  .then(response => {
    if (!response.ok) {
  ...
  fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos?language=en-US&api_key=${apiKey}`, options)
  .then(response => {
    if (!response.ok) {
  ...
  .then(response => {
    ...
    if (response.results && response.results.length > 0) {
      ...
      // 동영상이 없는 경우 메시지 표시
      document.getElementById("video-container").innerHTML = "이 영화에는 동영상이 없습니다.";
    }
  })
  .catch(err => console.error(err));
}